...
import { abrirModalBau } from './modais/bau/modal_bau.js';
import { abrirModalBanco } from './modais/banco/modal_banco.js';
import { abrirModalCorreio } from './modais/correio/modal_correio.js';
import { abrirModalHunts } from './modais/hunts/modal_hunts.js';
import { abrirModalMarket } from './modais/market/modal_market.js';
import { abrirModalMochila } from './modais/mochila/modal_mochila.js';
import { abrirModalRanking } from './modais/ranking/modal_ranking.js';
...
function aplicarEstilosMenu() {
    ...
    style.innerHTML = `
        ...
        .card-menu {
            ...
            user-select: none;
            flex-shrink: 0;
            position: relative; /* Importante para a bolinha ficar posicionada no card */
        }
        ...
        /* 🔴 ESTILO DA BOLINHA DE NOTIFICAÇÃO (PONTINHO VERMELHO) */
        .badge-notificacao {
            position: absolute;
            top: 4px;
            right: 4px;
            ...
            animation: pulsarBolinha 1.5s infinite ease-in-out;
        }

        @keyframes pulsarBolinha {
            0% { transform: scale(1); box-shadow: 0 0 4px #ff3333; }
            50% { transform: scale(1.3); box-shadow: 0 0 10px #ff0000; }
            100% { transform: scale(1); box-shadow: 0 0 4px #ff3333; }
        }
    `;
        ...
        container.innerHTML = `
            <div class="top-bar">
                ...
                <div class="menu-cards-container">
                    ...
                    <div id="btn-correio" class="card-menu">
                        <span id="badge-correio" class="badge-notificacao" style="display:none;"></span>
                        <img src="/img/icones/correio.png" alt="Correio" onerror="this.src='https://placehold.co/38x38/333/fff?text=Mail';">
...
export async function verificarNotificacaoCorreio() {
    const heroi = JSON.parse(localStorage.getItem('heroi'));
    if (!heroi || !heroi.id) return;

    try {
        const res = await fetch(`/api/correio/pendentes/${heroi.id}`);
        if (res.ok) {
            const data = await res.json();
            const badge = document.getElementById('badge-correio');
            if (badge) {
                badge.style.display = data.temPendentes ? 'block' : 'none';
            }
        }
    } catch (err) {
        console.error('Erro ao checar e-mails pendentes:', err);
    }
}
...
export function inicializarMenuSuperior() {
    ...
    document.getElementById('btn-correio')?.addEventListener('click', () => {
        ...
        verificarNotificacaoCorreio();
    });
    document.getElementById('btn-sair')?.addEventListener('click', sair);

    // Checa e-mails assim que inicia e verifica novamente a cada 15 segundos
    verificarNotificacaoCorreio();
    setInterval(verificarNotificacaoCorreio, 15000);
}